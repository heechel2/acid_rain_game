...
int enterHere=0;   
int timer=500000;   //time row down
int level=0;
char levelText[3]={0};
int score=0;
char scoreText[10]={0};
...
void gameover()
{
      ...
      fscanf(fp,"%s %d\n",t,&n);//this is highest score in file
      }
...
void savescore()
{
   ...
   {
           key = t[k].data;
           strcpy(keyname,t[k].sname);
           j=i-1;
        while(j>=0 &&t[j].data<key)
        {
              t[j+1] = t[j]; 
              j--;
        }
...
{   
   int i,j,k=0,n,key;
   int *data;
   char dataText[10]={0},iText[3]={0},t,tname[10];
   namescore *tt;
   FILE *fp;
   fp=fopen("score.txt","r");
   if(fp==NULL)
